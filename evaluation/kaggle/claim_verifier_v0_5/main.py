"""Generated Kaggle entry point for the claim-verifier benchmark."""
...
OUTPUT = Path("/kaggle/working/claim_verifier_v0_5")
ENV_ROOT = Path("/kaggle/working/claim_verifier_v0_5_venv")
ENV_PYTHON = ENV_ROOT / "bin" / "python"
CODE_ROOT = Path("/tmp/claim_verifier_v0_5_source")
REQUIREMENTS = Path("/tmp/claim-verifier-v0-5-requirements.txt")
VIRTUALENV_BOOTSTRAP = Path("/tmp/claim-verifier-v0-5-virtualenv")
...
print(json.dumps({"python": __import__("sys").version, "torch": torch.__version__,
 ...
 "pydantic": pydantic.__version__, "wrapt": wrapt.__version__}, sort_keys=True))
"""
...
def _run_benchmark() -> None:
    subprocess.run(
        [str(ENV_PYTHON), "-m", "scripts.run_claim_verifier_benchmark",
         "--suite", str(CODE_ROOT / "evaluation/suites/v0_5/claim_verifier_development.json"),
         "--output-dir", str(OUTPUT / "report"),
         "--model", "Qwen/Qwen3-4B",
         "--model-revision", "1cfa9a7208912126459214e8b04321603b3df60c",
         "--batch-size", "2", "--max-new-tokens", "1200", "--smoke-cases", "1"],
        check=True,
        cwd=CODE_ROOT,
    )
