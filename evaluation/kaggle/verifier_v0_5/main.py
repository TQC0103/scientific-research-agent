"""Generated Kaggle entry point for the controlled verifier benchmark."""
...
OUTPUT = Path("/kaggle/working/verifier_v0_5")
ENV_ROOT = Path("/kaggle/working/verifier_v0_5_venv")
ENV_PYTHON = ENV_ROOT / "bin" / "python"
CODE_ROOT = Path("/tmp/verifier_v0_5_source")
REQUIREMENTS = Path("/tmp/verifier-v0-5-requirements.txt")
VIRTUALENV_BOOTSTRAP = Path("/tmp/verifier-v0-5-virtualenv")
...
def _run_benchmark() -> None:
    subprocess.run(
        [
            str(ENV_PYTHON),
            "-m",
            "scripts.run_verifier_benchmark",
            "--definition",
            str(CODE_ROOT / "evaluation/suites/v0_5/verifier_development.json"),
            "--source-suite",
